...
const SendProposalModal = ({
  ...
  const generateDefaultProposalMessage = () => {
    ...
    const skills = candidate.skills?.slice(0, 3).map(s => s.skill_name || s).join(', ') || 'your skills';
...
[Your Company Name]`;
  ...
  const handleSendProposal = async () => {
    ...
    try {
      ...
      if (response.success) {
        Alert.alert(
          'Proposal Sent!',
          `Your job proposal has been sent to ${candidate.name || 'the candidate'}. You'll be notified when they respond.`,
          [
            {
              text: 'OK',
              onPress: () => {
        ...
        const errorMessage = response.errors?.[0] || response.message || 'Failed to send proposal';
  ...
  const getJobOptions = () => {
    return availableJobs.map(job => ({
      ...
      subtitle: `${job.employment_type} • ${job.work_mode} • ${job.positions_available} position${job.positions_available > 1 ? 's' : ''}`,
    }));
  };
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: theme.spacing.sm }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {candidate.position || candidate.current_title || 'Candidate'}
                    </Text>
                  </View>
            ...
            {isLoadingJobs ? (
              ...
              <>
                ...
                >
                  ...
                  >
                    Tip: Personalize your message by mentioning specific skills or experiences from the candidate's profile that caught your attention.
                  </Text>
                </View>
              </>
            )}
          ...
          {!isLoadingJobs && !errors.jobs && (
            ...
            >
              ...
              <TouchableOpacity
                ...
                disabled={isSendingProposal || !selectedJobId || !proposalMessage.trim()}
                style={{
                  flex: 2,
                  borderRadius: theme.borderRadius.lg,
                  overflow: 'hidden',
                  opacity: isSendingProposal || !selectedJobId || !proposalMessage.trim() ? 0.5 : 1,
                }}
                activeOpacity={0.9}
              >
                <LinearGradient
